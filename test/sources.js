...
test('get name from citation', function (t) {
  ...
  var repos = globiData.extractGithubRepos(sourceCitation);
...
test('get zenodo id from citation', function (t) {
  ...
  var repos = globiData.extractZenodoDOI(sourceCitation);
...
test('extract sources from json table', function (t) {
  t.plan(1);
  var sourceHash = globiData.parseSourceNames(someData);
  t.deepEqual(sourceHash, [{"name":"millerse/Dapstrom-integrated-database-and-portal-for-fish-stomach-records"},{"name":"millerse/Lice"}]);
});
